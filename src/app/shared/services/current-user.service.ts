...
export class CurrentUserService {
  readonly user$: Observable<UserDto> = this.authService.authSuccess$
    .pipe(
      switchMap(() =>
        this.usersService.usersUserIdGet(this.authService.user?.user_id!)
      ),
      shareReplay(1)
    );
  ...
  isAdmin(locationId?: string) {
    return !locationId
      ...
      : this.user$.pipe(map(user => this.isSuperAdmin || this.isCurrentLocationAdmin(locationId, user)));
  }
